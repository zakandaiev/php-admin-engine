...
class FilterBuilder
{
	...
	public function renderColSelected($selected)
	{
		...
		if (isset($selected['classifier'])) {
			$text = isClosure($selected['classifier']) ? $selected['classifier']($selected['value']) : __($selected['classifier'] . '.' . $selected['value']);
		}
		...
		$html .= '<a href="' . link_unfilter($selected['alias']) . '" class="label__close"><i class="ti ti-x"></i></a>';
