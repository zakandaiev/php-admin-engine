...
<?php if (count($languages) > 1) : ?>
  <div class="header__item dropdown dropdown_bottom-right">
    <img src="<?= pathResolveUrl(Asset::url(), lang('icon')) ?>" class="fit-cover radius-round" alt="<?= lang('locale') ?>">
    ...
    <div class="dropdown__menu">
      <?php foreach ($languages as $language) : ?>
        ...
        <a href="<?= pathResolveUrl(null, $language['key'], site('uri_full_no_language')) ?>" class="dropdown__item d-flex align-items-center gap-2">
          <img src="<?= pathResolveUrl(Asset::url(), lang('icon', $language['key'])) ?>" alt="<?= lang('locale', $language['key']) ?>" class="flex-shrink-0 d-inline-block h-1em">
