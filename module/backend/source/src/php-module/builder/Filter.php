...
class Filter
{
	...
	public function renderSelected($row_class = null, $row_attributes = null)
	{
		...
		$html .= '<a href="' . site('permalink') . '" class="d-inline-block mt-3">' . t('admin.filter.reset') . '</a>';
	...
	public function renderColSelected($selected)
	{
		...
		if (isset($selected['classifier'])) {
			$text = isClosure($selected['classifier']) ? $selected['classifier']($selected['value']) : t($selected['classifier'] . '.' . $selected['value']);
		}
		...
		$html .= '<a href="' . getLinkUnfilter($selected['alias']) . '" class="label__close"><i class="ti ti-x"></i></a>';
